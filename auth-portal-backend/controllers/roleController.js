...
const wrap = (fn, successStatus = 200) => async (req, res) => {
    try {
        ...
        if (req.method !== 'GET') {
            ...
            notifySystemChange(req.app, msg, req.user?.organizationId);
        }
...
const createRole = wrap((req) => roleService.createRole({ ...req.body, organizationId: req.user.organizationId }));
const getAllRoles = wrap((req) => roleService.getAllRoles(req.user.organizationId));
...
const createPermission = wrap((req) => roleService.createPermission({ ...req.body, organizationId: req.user.organizationId }));
const getAllPermissions = wrap((req) => roleService.getAllPermissions(req.user.organizationId));
